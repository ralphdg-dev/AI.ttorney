...
export const exportAdminAuditTrailPDF = async (
  auditLogs,
  adminFullName,
  adminEmail,
  adminInfo = null,
  ...
  try {
      ...
      try {
        ...
        if (parsedDetails.old_status && parsedDetails.new_status) {
          detailedAction = `Changed status from ${capitalizeFirst(
            parsedDetails.old_status
          )} to ${capitalizeFirst(parsedDetails.new_status)}`;
        }
        if (parsedDetails.old_role && parsedDetails.new_role) {
          detailedAction = `Changed role from ${capitalizeFirst(
            parsedDetails.old_role
          )} to ${capitalizeFirst(parsedDetails.new_role)}`;
        }
      } catch {
    ...
    // Save the PDF
    const fileName = `Admin_Audit_Trail_${
      adminFullName?.replace(/\s+/g, "_") || "Unknown"
    }_${reportNumber}.pdf`;
    doc.save(fileName);
    ...
    if (adminId) {
      ...
      console.warn(
        "Admin Audit Trail PDF generated but no adminId provided for audit logging"
      );
    }
  } catch (error) {
...
export const exportAdminActivityPDF = async (
  activityLogs,
  adminFullName,
  adminEmail,
  adminInfo = null,
  ...
  try {
    ...
    // Save the PDF
    const fileName = `Admin_Activity_Report_${
      adminFullName?.replace(/\s+/g, "_") || "Unknown"
    }_${reportNumber}.pdf`;
    doc.save(fileName);
    ...
    if (adminId) {
      ...
      console.warn(
        "Admin Activity Report PDF generated but no adminId provided for audit logging"
      );
    }
  } catch (error) {
...
export { addPDFHeader, addAdminInfo, addPDFFooter };
