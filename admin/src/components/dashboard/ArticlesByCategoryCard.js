      ...
      <div className="flex-1 min-h-[200px] sm:min-h-[250px] md:min-h-[300px]">
        {data.length > 0 ? (
            ...
            <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
              <defs>
                {data.map((entry, index) => (
                  <linearGradient key={`articles-gradient-${index}`} id={`articles-gradient-${index}`} x1="0" y1="0" x2="0" y2="1">
              ...
              >
                {data.map((entry, index) => (
                  <Cell 
                    key={`cell-${index}`} 
                    fill={`url(#articles-gradient-${index})`}
                  />
                ))}
              </Bar>
            </BarChart>
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-50 to-amber-100 flex items-center justify-center mb-3">
              <span className="text-2xl text-amber-400">📝</span>
            </div>
            <p className="text-gray-400 text-sm font-medium">No articles found</p>
            <p className="text-gray-300 text-xs mt-1">Share legal insights with the community</p>
          </div>
        )}
      </div>
    </div>
