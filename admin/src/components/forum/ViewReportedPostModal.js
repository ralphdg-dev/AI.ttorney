...
import { MessageSquare, User, History, Loader2, Flag } from 'lucide-react';
...
const ViewReportedPostModal = ({ open, onClose, report, loading = false }) => {
      ...
      const mockLogs = [
        {
          ...
          role: 'user',
          date: report.created_at || report.submitted_at,
        },
      ];
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-700">{formatDate(report.created_at || report.submitted_at)}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {report.reason_context && (
                <div>
                  ...
                  <div className="text-xs text-gray-700">{report.reason_context}</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-lg border border-gray-200 overflow-hidden">
            <div className="bg-gray-100 px-4 py-2 border-b border-gray-200">
              <div className="flex items-center gap-2">
                ...
                <div className="text-[10px] font-semibold text-gray-700 uppercase tracking-wide">Reported Post</div>
              </div>
            </div>
            <div className="p-4">
              ...
              <div className="max-h-32 overflow-y-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      <th className="px-2 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase tracking-wider">Action</th>
                      <th className="px-2 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase tracking-wider">Performed By</th>
                      <th className="px-2 py-1.5 text-left text-[9px] font-medium text-gray-500 uppercase tracking-wider">Date</th>
                    </tr>
                  </thead>
                    ...
                    {auditLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-gray-50">
                        <td className="px-2 py-1.5">
                          <div className="text-[9px] font-medium text-gray-900">{log.action}</div>
                        </td>
                        <td className="px-2 py-1.5 whitespace-nowrap">
                          <div className="text-[9px]">
                            <div className="font-medium text-gray-900">{log.admin || 'Unknown'}</div>
                            <div className="text-gray-500 capitalize">{log.role || 'User'}</div>
                          </div>
                        </td>
                        <td className="px-2 py-1.5 whitespace-nowrap text-[9px] text-gray-500">{formatDate(log.date)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
export default ViewReportedPostModal;
