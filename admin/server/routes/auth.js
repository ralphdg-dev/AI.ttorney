...
router.get('/maintenance', authenticateAdmin, async (_req, res) => {
  try {
    ...
    // Compute effective maintenance status based on schedule window
    let maintenance = row;
    if (row) {
      const now = new Date();
      ...
      const withinWindow = hasSchedule && (
        (start && end && now >= start && now < end) ||
        (start && !end && now >= start) ||
        (!start && end && now < end)
      );

      const storedIsActive = Boolean(row.is_active);
      const effectiveIsActive = hasSchedule ? withinWindow : storedIsActive;

      // If we are past the end of a full window, clear the schedule and deactivate
      const pastWindow = start && end && now >= end;
      ...
      try {
        if (pastWindow) {
          const { error: clearError } = await supabaseAdmin
            ...
            .eq('id', row.id);
          if (clearError) {
            console.error('Failed to clear maintenance schedule after end_time:', clearError);
          }
          ...
          // Persist the auto-toggled state back to the DB so Supabase shows the current status
          if (effectiveIsActive !== storedIsActive) {
            const { error: updateError } = await supabaseAdmin
              .from('system_maintenance')
              .update({ is_active: effectiveIsActive })
              .eq('id', row.id);
            if (updateError) {
              console.error('Failed to auto-update maintenance is_active flag:', updateError);
            }
          }
        ...
        console.error('Unexpected error while updating maintenance flags:', updateErr);
        maintenance = {
          ...row,
          is_active: effectiveIsActive,
          effective_is_active: effectiveIsActive,
...
try {
  ...
  let result;
  const previousRow = rows && rows.length > 0 ? rows[0] : null;
  if (previousRow) {
    const id = previousRow.id;
    const { data, error } = await supabaseAdmin
      .from('system_maintenance')
      .update(payload)
      .eq('id', id)
  ...
  // Create in-app notifications for all users when maintenance schedule is set/changed
  try {
    const scheduleChanged =
      (payload.start_time || payload.end_time) &&
      (!previousRow ||
        previousRow.start_time !== payload.start_time ||
        previousRow.end_time !== payload.end_time);
    ...
    if (scheduleChanged) {
      ...
      if (!usersError && Array.isArray(users) && users.length > 0) {
        const hasStart = !!payload.start_time;
        const hasEnd = !!payload.end_time;
        ...
        const startLabel = hasStart ? formatDisplayTime(payload.start_time) : '';
        const endLabel = hasEnd ? formatDisplayTime(payload.end_time) : '';

        const defaultMessage = hasStart
          ? hasEnd
            ? `The system will undergo maintenance from ${startLabel} to ${endLabel}.`
            : `The system will undergo maintenance from ${startLabel}.`
          : 'The system will undergo maintenance soon.';
        ...
        if (notifError) {
          console.error('Failed to create maintenance notifications:', notifError);
        }
      }
    }
  } catch (notifyErr) {
    console.error('Unexpected error while creating maintenance notifications:', notifyErr);
  }
