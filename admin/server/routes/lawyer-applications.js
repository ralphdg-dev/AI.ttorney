...
router.get("/", authenticateAdmin, async (req, res) => {
  try {
    ...
    if (search) {
      ...
      const { data: searchApplications, error: searchError } =
        await supabaseAdmin
          .from("lawyer_applications")
          .select(
            `
...
router.get("/:id/history", authenticateAdmin, async (req, res) => {
  try {
    ...
    const { data: currentApplication, error: currentError } =
      await supabaseAdmin
        .from("lawyer_applications")
        .select("user_id")
        .eq("id", id)
    ...
    const transformedApplications =
      applications?.map((app) => ({
        ...
        admin_name:
          adminMap.get(app.reviewed_by)?.full_name ||
          adminMap.get(app.reviewed_by)?.email ||
          null,
  ...
  try {
    ...
    if (error || !application) {
      return res.status(500).json({
        success: false,
        error:
          "Lawyer application update failed: " +
    ...
    if (
      ...
      filteredUpdateData.status !== existingApp.status
    ) {
      changes.push({
        action: `Status changed from "${existingApp.status || "none"}" to "${
          filteredUpdateData.status
        }"`,
        field: "status",
    ...
    if (
      filteredUpdateData.admin_notes !== undefined &&
      filteredUpdateData.admin_notes !== (existingApp.admin_notes || "")
    ) {
    ...
    if (
      filteredUpdateData.roll_number &&
      filteredUpdateData.roll_number !== existingApp.roll_number
    ) {
      changes.push({
        action: `Roll number changed from "${
          existingApp.roll_number || "none"
        }" to "${filteredUpdateData.roll_number}"`,
        field: "roll_number",
    ...
    if (
      filteredUpdateData.roll_signing_date &&
      filteredUpdateData.roll_signing_date !== existingApp.roll_signing_date
    ) {
      changes.push({
        action: `Roll signing date changed from "${
          existingApp.roll_signing_date || "none"
        }" to "${filteredUpdateData.roll_signing_date}"`,
  ...
  try {
    ...
    if (!["pending", "accepted", "rejected", "resubmission"].includes(status)) {
      return res.status(400).json({
        success: false,
        error:
          "Invalid status. Must be one of: pending, accepted, rejected, resubmission",
      });
    }
    ...
    // Send notification to the applicant when their application is accepted or rejected
    if (status === "accepted" || status === "rejected") {
      try {
        ...
        const message = isAccepted
          ? "Your lawyer application has been accepted. You now have access to lawyer features in AI.ttorney."
          : "Your lawyer application has been reviewed and was not approved at this time. You may review the feedback and resubmit if allowed.";
        ...
        const { error: notifyError } = await supabaseAdmin
          ...
          .insert([
            {
              user_id: application.user_id,
              type: "lawyer_application_decision",
              title,
              message,
              data: {
        ...
        return res.status(500).json({
          success: false,
          error: "Failed to notify user about lawyer application decision",
        });
      }
    }
...
router.post("/signed-url", authenticateAdmin, async (req, res) => {
  try {
    ...
    const fileExists =
      fileData &&
      fileData.some(
        (file) => filePath.includes(file.name) || filePath.endsWith(file.name)
      );
