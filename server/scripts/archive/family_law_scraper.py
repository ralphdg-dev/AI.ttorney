...
def scrape_family_code(url):
    ...
    logger.info("Fetching Family Code content...")
    ...
    for p in all_paragraphs:
        ...
        if title_match:
            ...
            logger.info(f"Found: {current_title}")
            continue
        ...
        if article_match:
            ...
            logger.info(f"Article {article_number} - {topic}: {title[:50]}...")
            if current_title or current_chapter:
                logger.info(f"   Hierarchy: {current_title} | {current_chapter}")
    
    return articles

def scrape_ra_9262(url):
    """Scrape RA 9262 (VAWC) from LawPhil"""
    logger.info("Fetching RA 9262 content...")
    try:
        response = requests.get(url)
        response.encoding = 'utf-8'
        soup = BeautifulSoup(response.text, "html.parser")
        ...
        for p in all_paragraphs:
            ...
            if section_match:
                ...
                logger.info(f"Section {section_number} - {topic}: {title[:50]}...")
                if current_title or current_chapter:
                    logger.info(f"   Hierarchy: {current_title} | {current_chapter}")
        ...
        logger.error(f"Error scraping RA 9262: {e}")
        return []
...
def display_summary(data):
    ...
    logger.info("**Laws:**")
    for law in data['laws']:
        logger.info(f"  • {law['name']} ({law['date']})")
    
    stats = data['statistics']
    logger.info(f"**Total:** {stats['total_entries']} entries")
    logger.info(f"  • Family Code: {stats['family_code_articles']} articles")
    logger.info(f"  • RA 9262: {stats['ra_9262_sections']} sections")
    
    logger.info("**By Topic:**")
    for topic, count in stats['by_topic'].items():
        logger.info(f"  • {topic}: {count} entries")
        
                             
        sample_entries = data['topics'][topic][:2]
        for entry in sample_entries:
            logger.info(f"    - {entry['type']} {entry['number']}: {entry['title'][:50]}...")

if __name__ == "__main__":
    logger.info("CLEAN FAMILY LAW SCRAPER")
    logger.info("Simplified Structure - No Redundancy")
    logger.info("=" * 45)
    ...
    logger.info("**CLEAN SCRAPING COMPLETE!**")
    logger.info("Files generated:")
    logger.info("  • clean_family_law.json - Clean structured data")
    logger.info("  • clean_family_law.md - Readable documentation")
    logger.info("Sources:")
    logger.info(f"  • {FAMILY_CODE_URL}")
    logger.info(f"  • {RA_9262_URL}")
