...
const LawyerConsultPage: React.FC = () => {
  ...
  const fetchConsultationRequests = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL || 'http://localhost:8000'}/api/consult-actions/my-consultations${
  ...
  const fetchStats = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL || 'http://localhost:8000'}/api/consult-actions/stats`,
        {
          headers: {
            'Authorization': `Bearer ${session?.access_token}`,
            'Content-Type': 'application/json',
  ...
  const handleRejectRequest = (requestId: string, clientName: string, event?: any) => {
    if (event) {
      event.stopPropagation();
    }
    setConfirmationModal({
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL || 'http://localhost:8000'}/api/consult-actions/${confirmationModal.requestId}/${endpoint}`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${session?.access_token}`,
      ...
      if (response.ok) {
        ...
        await fetchStats();
        Alert.alert('Success', `Consultation ${confirmationModal.actionType}ed successfully`);
      } else {
        Alert.alert('Error', `Failed to ${confirmationModal.actionType} consultation`);
      }
    } catch (error) {
          ...
          <View style={tw`flex-row flex-wrap -mr-3`}>
            <View style={[tw`bg-white rounded-xl p-4 flex-1 border border-gray-100 mr-3 mb-3`, { minWidth: 144, boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }]}>
              <View style={tw`flex-row items-center justify-between mb-2`}>
                ...
                <Text style={tw`text-2xl font-bold text-gray-900`}>{stats.pending_requests}</Text>
              </View>
            ...
            <View style={[tw`bg-white rounded-xl p-4 flex-1 border border-gray-100 mr-3 mb-3`, { minWidth: 144, boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }]}>
              <View style={tw`flex-row items-center justify-between mb-2`}>
                ...
                <Text style={tw`text-2xl font-bold text-gray-900`}>{stats.today_sessions}</Text>
              </View>
          ...
          <View style={tw`flex-row flex-wrap -mr-3 mt-3`}>
            <View style={[tw`bg-white rounded-xl p-4 flex-1 border border-gray-100 mr-3`, { minWidth: 144, boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }]}>
              <View style={tw`flex-row items-center justify-between mb-2`}>
                ...
                <Text style={tw`text-2xl font-bold text-gray-900`}>{stats.completed_requests}</Text>
              </View>
            ...
            <View style={[tw`bg-white rounded-xl p-4 flex-1 border border-gray-100 mr-3`, { minWidth: 144, boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }]}>
              <View style={tw`flex-row items-center justify-between mb-2`}>
                ...
                <Text style={tw`text-2xl font-bold text-gray-900`}>{stats.total_requests}</Text>
              </View>
        ...
        <View style={tw`px-4`}>
          ...
          {consultationRequests.length === 0 ? (
            <View style={tw`bg-white rounded-2xl p-8 items-center justify-center border border-gray-100`}>
              <Text style={tw`text-lg font-semibold text-gray-500 mb-2`}>No consultation requests found</Text>
                ...
                >
                  ...
                  <View style={tw`flex-row items-start justify-between mb-4`}>
                    <View style={tw`flex-row items-center flex-1 mr-3`}>
                      ...
                      <View style={tw`ml-3 flex-1`}>
                        ...
                        <Text style={tw`text-sm text-gray-600 font-medium`} accessibilityLabel={`Consultation request from ${request.client_name}`}>
                  ...
                  {request.status === 'pending' && (
                    <View style={tw`flex-row gap-2`}>
                      <TouchableOpacity
                        style={[tw`flex-1 py-3 rounded-xl mr-3`, { backgroundColor: '#EF4444', boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }]}
                        onPress={(event) => handleRejectRequest(request.id, request.client_name, event)}
                      ...
                      <TouchableOpacity
                        style={[tw`flex-1 py-3 rounded-xl`, { backgroundColor: Colors.primary.blue, boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }]}
                        onPress={(event) => handleAcceptRequest(request.id, request.client_name, event)}
                  ...
                  {request.status === 'accepted' && (
                    <TouchableOpacity 
                      ...
                      onPress={(event) => handleCompleteRequest(request.id, request.client_name, event)}
